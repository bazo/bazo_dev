...
//require_once dirname(__FILE__) . '/Nette/Loaders/AutoLoader.php';

//require_once dirname(__FILE__) . '/Nette/Framework.php';
 ...
 * @package    Nette\Loaders
 */
class MokujiLoader extends AutoLoader
{
	/** @var array */
	public $scanDirs;
	...
	public function rebuild($force = TRUE)
	{
		...
		if ($force || !$this->rebuilded) {
			foreach (array_unique($this->scanDirs) as $dir) {
                ...
                $listCache['classList'] = $this->class_list;
	 ...
	 * @return void
	 */
	public function addClass($class_name, $file)
	{
		...
		if (!empty($this->list[$class]) && $this->list[$class] !== $file) {
			...
			throw new InvalidStateException("Ambiguous class '$class' resolution; defined in $file and in " . $this->list[$class] . ".");
                ...
                $this->class_list[$class_name] = $file;
